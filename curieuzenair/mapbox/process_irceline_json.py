...
input_file = 'no2_anmean_station_brussels.json'
output_file = 'no2_anmean_station_brussels_2021_mapbox.json'
radius = 10
...
def process_geojson(input_file, output_file, radius):
    ...
    # Prepare an empty list for filtered and processed features
    processed_features = []

    # Loop through each feature in the input GeoJSON
    for feature in data['features']:
        # Extract year and geometry
        year = feature['properties']['year']
        geom = feature['geometry']

        # Check if the feature is from the year 2021
        if year.startswith('2021'):
            # Replace Point geometry with a Hexagon (radius = 1 meter)
            if geom['type'] == 'Point':
                coordinates = geom['coordinates']
                # Create a hexagon geometry with a 1m radius
                hexagon_geom = create_hexagon(coordinates, radius=radius)
                # Add the new hexagonal feature to the processed list
                processed_features.append(Feature(geometry=hexagon_geom.__geo_interface__, properties=feature['properties']))
...
print(f"Filtered and processed GeoJSON saved to {output_file}")